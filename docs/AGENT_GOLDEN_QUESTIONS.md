...
| Does the frontend composition resolver make every route or plugin dynamically available? | No. The resolver currently covers build-included Guest Checkout and selected marketplace route/navigation eligibility. Product and cart views also share provisional action and summary semantics through host-rendering contracts, but entity, seller, authentication, currency, channel, storage, and navigation policy stays host-owned; generic provider, workflow, action, browser-extension, and dynamic-plugin composition remains outside the current contract. | `/build/runtime-capabilities` |
...
| Does static Order Extension v1 mean one universal module platform is shipped? | No. Trusted payment modules and static Order Extension v1 are separate family-specific slices. The named static slices include profile selection, lifecycle health, and fail-closed admission, but they do not share one descriptor, lifecycle, or manager; contextual tenant/resource policy, drain and upgrade orchestration, third-party process runtimes, and Wasm Functions remain incomplete, planned, or deferred. | `/build/extensions` |
