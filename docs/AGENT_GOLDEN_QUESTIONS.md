...
| Does the frontend composition resolver make every route or plugin dynamically available? | No. The current slice resolves only build-included Guest Checkout and selected marketplace route/navigation eligibility against supported profiles and backend capabilities. Providers, workflows, actions, browser-extension adoption, and dynamic plugins remain outside the current contract. | `/build/runtime-capabilities` |
